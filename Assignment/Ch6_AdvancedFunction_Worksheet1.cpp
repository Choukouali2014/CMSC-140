...
int main()
{
	...
	------------------------------------------------------------------------------
	*/
	//Yes a function can return a boolean data type but its returned values will be 0 for false and 1 for true
	...
	*/
	...
	1. Can a local variable be used outside its scope?
		No, the local variable cannot be used outside of its scope
	...
	When a  variable can be used anywhere in the file


	4. What does it mean to shadow a global variable?
	shadow a global variable is to have a local variable or parameter variable with the same name as a global variable.
	-----------------------------------------------------
	*/
	...
	*/
	...
	Does the data a static variable get erased when the function ends? No it doesn't get erased when the function ends;
	------------------------------------------------------------------
	*/
	...
	*/
	/* the output is : 
		...
		The function has been called : 1 time(s).
		The function has been called : 2 time(s).
		The function has been called : 3 time(s).
		The function has been called : 4 time(s).
		The function has been called : 5 time(s).
		The function has been called : 6 time(s).
		The function has been called : 7 time(s).
		The function has been called : 8 time(s).
		The function has been called : 9 time(s).
		The function has been called : 10 time(s).
	...
	*/
	...
	2. Where do you define the default arguments?
		The default arguments are usually listed un the function prototype.
	3. If a function does not have a prototype, where do you define the default arguments?
			default arguments may be specified in the function header
	4. If a function has default arguments for each parameter, can you call it without
	   passing any values? Yes we can call the function without passing any values
	--------------------------------------------------------------------------------------
	*/
...
double circumference(double diametre)
{
	const double PI = 3.1415;
	return diametre * PI;
}
